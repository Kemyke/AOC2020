...
namespace AOC202002
{
    class Program
    {
        static string input = @"2-7 p: pbhhzpmppb
3-6 h: jkhnhwhx
5-6 x: xxxxxmxf
...
4-5 k: kkxxkn";
        ...
        static void Main(string[] args)
        {
            ...
            {
                var dash = l.IndexOf('-');
                int min = int.Parse(l.Substring(0,dash));
                var fs = l.IndexOf(" ");
                int max = int.Parse(l.Substring(dash+1, fs-dash-1));
                string ch = l.Substring(fs + 1, l.IndexOf(":") - fs -1);

                
                string pwd = l.Substring(l.IndexOf(":") + 2);

                var ch1 = pwd[min - 1].ToString();
                var ch2 = pwd[max - 1].ToString();
